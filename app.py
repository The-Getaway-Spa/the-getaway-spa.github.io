...
# Temporary in-memory store (populated from disk at startup)
lessons = []


def load_existing_lessons():
  """Scan the lessons directory and populate the in-memory `lessons` list.
  This ensures lessons persist across server restarts even though `lessons`
  is an in-memory structure used by the API.
  """
  os.makedirs(LESSONS_DIR, exist_ok=True)
  for fname in os.listdir(LESSONS_DIR):
    if not fname.lower().endswith('.html'):
      continue
    lesson_id = os.path.splitext(fname)[0]
    file_path = os.path.join(LESSONS_DIR, fname)
    title = lesson_id.replace('-', ' ').replace('_', ' ').title()
    try:
      ...
      # ignore read errors and fall back to filename-derived title
      pass

    lesson_path = f"lessons/{fname}"
    # avoid duplicates
    if not any(l.get('id') == lesson_id for l in lessons):
      lessons.append({"id": lesson_id, "title": title, "path": lesson_path})


# Populate lessons from disk when the server starts
...
def delete_lesson(lesson_id):
  ...
  # Also remove the lesson file from disk if it exists
  ...
  try:
    ...
    app.logger.exception("Failed to remove lesson file %s: %s", file_path, e)
